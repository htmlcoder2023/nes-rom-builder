...
while satisfied == False and i < possibilities:
    ...
    while bytesWritten < numStop:
        if len(sameBytes) == 0:
            byteString = random.choices(bytesArr, k = numStop)
            prg.write(bytearray(byteString))
            bytesWritten += numStop
        elif len(sameBytes) > 0:
            if bytesLoc[prgCounter] - bytesWritten > 0:
                byteString = random.choices(bytesArr, k = bytesLoc[prgCounter] - bytesWritten)
                prg.write(bytearray(byteString))
                prg.write(bytearray(bytesArr[sameBytes[prgCounter]]))
                bytesWritten = bytesLoc[prgCounter] + 1
                prgCounter += 1
            else:
                try:
